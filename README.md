...
| `A3_AGENT_SIGNING_KEY` | 采集器发布签名密钥（ed25519 seed 的 base64）；留空则自动生成并持久化（重启后代际稳定）；显式设置时优先且不写文件 | 空(自动生成+持久化) |
...
- **验签能力分层**：Linux 的 OpenSSL 3 支持 ed25519 验签——安装期内联 `openssl pkeyutl -verify`，
  ...
  显式警告 + 指纹核对（指纹见安装输出与 `/api/v1/setup-info`），真正常驻的强制关卡是装完后的
  `a3-agent doctor` 签名自检（全平台，校验固定安装路径字节）。
- **代际签名**：服务端对每个发布产物签名一次并按内容缓存——产物文件在签名后被改动而未重启发布
  时，`.sig` 请求返回 409 并留服务端日志（视为篡改），客户端拿到旧签名对不上新字节即硬失败。
- **doctor 复核**：`a3-agent doctor` 除原有体检外新增「发布签名」项，校验二进制/签名/公钥三件一致
  （pre-P2 存量安装无签名布局时按 [跳过] 处理，不误报）；有 `.prev` 时提示可回滚。

诚实边界：签名防止的是**服务端产物目录被植毒/篡改/误覆盖**后的安装与常驻字节漂移；纯 HTTP 下整个
通道可被 MITM（脚本内嵌公钥会同被替换），同权限本地对手也能连同公钥一起覆盖——这两类不在防御范围。
